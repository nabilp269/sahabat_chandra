...
export default function ForumNotificationPopup() {
    ...
    const checkNew = async () => {
        try {
            const res = await fetch(route("forum.check-new"), {
                headers: { Accept: "application/json" },
            });
            const data = await res.json();

            if (!data.has_new || !data.post) return;

            const dismissedId = localStorage.getItem("forum_dismissed_post_id");
            if (String(data.post.id) === dismissedId) return;
    ...
    const handleClose = () => {
        if (popup) {
            localStorage.setItem("forum_dismissed_post_id", String(popup.id));
        }
        setPopup(null);
    };
