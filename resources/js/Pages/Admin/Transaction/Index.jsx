...
export default function Index({ transactions: initial }) {
    ...
    useEffect(() => {
        ...
        channel.listen(".TransactionCreated", (e) => {
            setTrxList((prev) => [e.transaction, ...prev]);
        });

        channel.listen(".TransactionStatusUpdated", (e) => {
            setTrxList((prev) =>
                prev.map((t) => (t.id === e.transaction.id ? { ...t, ...e.transaction } : t))
            );
        });
    ...
    const filteredTransactions = useMemo(() => {
        ...
        return trxList.filter((trx) =>
            ...
            (trx.user?.name ?? "").toLowerCase().includes(keyword) ||
            (trx.receiver_name ?? "").toLowerCase().includes(keyword)
        );
    }, [trxList, search]);
        ...
        <AdminLayout>
            ...
            <div className="space-y-6">
                ...
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-800">Data Transaksi</h1>
                        <p className="text-gray-500 mt-2">Semua transaksi pengguna Sahabat Chandra.</p>
                    </div>
                    <div className="bg-blue-600 text-white px-5 py-3 rounded-2xl shadow-lg flex items-center gap-3">
                        <CreditCard size={22} />
                        <div>
                            <p className="text-sm">Total</p>
                            <h3 className="text-xl font-bold">{trxList.length}</h3>
                        </div>
                    </div>
                </div>
                    ...
                    <table className="w-full">
                        <thead className="bg-[#0057B8] text-white">
                            <tr>
                                ...
                                <th className="px-5 py-4 text-left">Pengirim</th>
                                <th className="px-5 py-4 text-left">Penerima</th>
                                <th className="px-5 py-4 text-left">HKD</th>
                        ...
                        <tbody>
                            {filteredTransactions.length > 0 ? (
                                filteredTransactions.map((trx) => (
                                    <tr key={trx.id} className="border-b hover:bg-blue-50 transition">
                                        <td className="px-5 py-4 font-semibold">#{trx.id}</td>
                                        <td className="px-5 py-4">
                                            <p className="font-semibold">{trx.user?.name ?? "-"}</p>
                                            <p className="text-sm text-gray-500">{trx.user?.email}</p>
                                        </td>
                                        <td className="px-5 py-4">{trx.receiver_name}</td>
                                        <td className="px-5 py-4 font-semibold text-blue-600">
                                            HK$ {trx.amount_hkd}
                                        </td>
                                        <td className="px-5 py-4">{badgeStatus(trx.status)}</td>
                                        <td className="px-5 py-4 text-center">
                                            <Link
                                                href={route("admin.transaction.show", trx.id)}
                                ...
                                <tr>
                                    <td colSpan={6} className="py-16 text-center">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <h2 className="mt-4 text-xl font-bold text-gray-600">Tidak ada transaksi</h2>
                                            <p className="text-gray-400 mt-2">Belum ada data transaksi yang ditemukan.</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
                ...
                <div className="flex justify-between items-center text-sm text-gray-500">
                    <span>
                        Total Transaksi :
                        <span className="font-bold text-blue-600 ml-2">{filteredTransactions.length}</span>
                    </span>
                    <span>Sahabat Chandra Admin Panel</span>
                </div>
